...
You need to defeat all $n$ dragons.

To defeat the $i$-th dragon you need to have an army with **at least** $a_i$ soldiers.

After you defeat the $i$-th dragon, your army size **decreases** by $b_i$ soldiers ($b_i \le a_i)$.

You can defeat dragons in any order.

Find the minimal initial army size $S$ to defeat all dragons.

### Solution

- Lets $a_i$ be the $i$-th dragon.
- Function $F(a)$ is the minimal initial army size to defeat all dragons, if you defeat them in order of $a$.
- Lets $a_j$ is the next dragon for the $a_i$ in the optimal order.
  - Why is it better to have $a_i$ before $a_j$ than $a_j$ before $a_i$?
    - Lets we already defeated some dragons before $a_i$ and $a_j$ and lost $P$ soldiers.
    - How many soldiers do we need before we fight both $a_i$ and $a_j$?
      - In the first case we need to have at least $\max(a_i, a_j + b_i)$ soldiers.
      - In the second case we need to have at least $\max(a_j, a_i + b_j)$ soldiers.
    - We want to minimize $S$, so we want to minimize $S - P$ as well.
      - But $S - P$ should be at least the number of soldiers we need before we fight both $a_i$ and $a_j$.
      - So it's better to choose the order where needed number of soldiers is smaller.
- So the **comparator** you need is $\max(a_i, a_j + b_i) < \max(a_j, a_i + b_j)$.
- Function $F$ can be implemented with $O(n)$ complexity:
  - Start with $S = 0$ and current army size $C = 0$.
  - If $C$ is less than $a_i$, increase $S$ and $C$ by $(a_i - C)$.
  - After defeating the dragon decrease $C$ by $b_i$.
- Total complexity is $O(n \log n)$.
...
```cpp
auto get_ans = [&]() {
    auto order = dragons; // dragons is vector of structs (need, killed)
    std::sort(order.begin(), order.end(),
        // < 
        [](const auto& first, const auto& second) {
...
- Classical "salesman" problem:
  ...
  - Consider we have complete graph (roads between any two cities). 
