...
**Important**: your amount of coins **can't be negative** at any moment.
...
- In the classical "knapsack" problem items also have weights $w_i$.
  - It's well known that this problem can be solved only with dynamic programming.
  - Find counterexample that greedy approach with comparator $c_i > c_j$ does not work in this case.
- In the problem "The Dragon Land", we actually skipped one detail: 
  - we found subset of dragons to defeat, but in order that was "comfortable" for us;
  - but by the problem statement we need to defeat dragons in the order they are located on the line.
  - **prove** that it is always possible to defeat selected dragons in the original order without going to the negative amount of coins in the process.
    - Use the inequality $k \le a_{i_k} \le a_{i_{k - 1}} \le \ldots$.
- Let's modify the problem "Dragons and Princesses":
  - now princess $j$ will forcefully marry you if your total number of coins is at least $a_j$.
  - **find** counterexample that greedy approach with "undefeating" dragons with the smallest $a_i$ does not work in this case.
  - **prove** that this variation of problem can't be solved with greedy approach at all.
    - reduce problem to the one you know that can't be solved by greedy approach.
