...
- if the value $k$ in the current node equals $x$ and the `stop_on_equal` flag is set - the search is complete, we return `last`;
...
Example implementation of the `next` function for LVR traversal has already been described in the section on [non-recursive traversals of binary trees](./../../tree/2.Concept.Tree.Binary.md#non-recursive-variant-of-full-traversal).
...
It is necessary to delete node with the given value $val$.
...
- Otherwise, we will find node $x$, which follows $v$ in the order of traversal.
  - At this point we know that node $v$ has two children, so node $x$ is actually "the leftmost in the right subtree of $v$" 
    - And this mean that node $x$ cannot have a left child (otherwise, it wouldn't be the leftmost).
- We will swap the data from $x$ to $v$, after which we will **recursively** call the deletion of $x$ from the right subtree of node $v$.
  - Note that now node $x$ has at most one child, so the recursive call will fall into the simpler case.
